---
title: "Transactions - Knot | Plaid Docs"
source_url: "https://plaid.com/docs/transactions/partnerships/knot/"
scraped_at: "2026-03-07T22:05:21+00:00"
---

# Add Knot to your app

#### Instantly make your card top-of-wallet at merchants with Knot’s API

![Partnership Knot logo](/assets/img/docs/plaid-knot-partnership.png)

  

#### Overview

Knot and Plaid have partnered to offer a seamless and secure solution for managing card-on-file information across a vast network of merchants. Knot's CardSwitcher™ effortlessly automates the process of adding or updating a cardholder’s credit or debit card at almost any merchant, ensuring that your card remains top-of-wallet. Knot Customers can use Plaid’s transaction data to suggest hyper relevant CardSwitches that are personalized based on a cardholder’s transaction history.

Utilizing Knot’s CardSwitcher™, card issuers can now automate the entire lifecycle of a cardholder. From initial issuance, through card replacements, and card expirations, all while remaining top-of-wallet. The integration with Plaid further enhances this process by providing Knot customers with transaction history that allows cardholders to receive personalized CardSwitcher suggestions.

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Knot account](https://knotapi.com/products/contact-sales?utm=plaid) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Knot `processor_token`, which allows you to quickly and securely retrieve the user's financial data via [Knot's API](https://docs.knotapi.com/docs). Utilizing Plaid + Knot enables a seamless workflow for connecting external financial accounts to Knot.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Knot in order to use the
Plaid + Knot integration. You'll also need to enable your
Plaid account for the Knot integration.

First, you will need to work with the Knot team
to [sign up for a Knot account](https://knotapi.com/products/contact-sales?utm=plaid),
...
for Knot to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Knot to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Knot.
...
the Link `public_token` for a Plaid `access_token` and a Knot `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. The `accounts` array will contain information about bank accounts associated with the credentials entered by the user, and may contain multiple accounts if the user has more than one bank account at the institution. In most cases, you will want your users to connect all of their accounts so that they can be tracked in Knot.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Knot `processor_token`. You'll send this token
to Knot and they will use it to securely retrieve transaction data from Plaid.

You can create Knot `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Knot Production credentials
...
You will need Knot Production credentials prior to initiating live traffic
in the Knot API with Plaid.
