---
title: "Transactions - Boom | Plaid Docs"
source_url: "https://plaid.com/docs/transactions/partnerships/boom/"
scraped_at: "2026-03-07T22:05:20+00:00"
---

# Add Boom to your app
...
![Partnership Boom logo](/assets/img/docs/plaid-boom-partnership.png)

  

#### Overview

Plaid and Boom have partnered to offer businesses a secure and streamlined way for customers to build their credit by reporting current and past rent payments to the 3 largest U.S. credit bureaus (Experian, Equifax, and Transunion).

With the Plaid + Boom integration, use Plaid Link to instantly connect customer bank accounts and detect rent payments. Using the Plaid processor token, Boom can detect customer rent payments from accounts customers have already linked directly to your platform. Customers only need to link their bank account once, enabling you to provide rent reporting as a streamlined, convenient amenity to customers.

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Boom account](https://www.boompay.app/rent-reporting-as-a-service) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Boom `processor_token`, which allows you to quickly and securely verify a bank funding source via [Boom's API](https://docs.boompay.app/plaid-processor-token-integration) without having to store any sensitive banking information. Utilizing Plaid + Boom enables a seamless workflow for connecting external financial accounts to Boom.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Boom in order to use the
Plaid + Boom integration. You'll also need to enable your
Plaid account for the Boom integration.

First, you will need to work with the Boom team
to [sign up for a Boom account](https://www.boompay.app/rent-reporting-as-a-service),
...
for Boom to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Boom to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Boom.
...
the Link `public_token` for a Plaid `access_token` and a Boom `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. If you want the user to specify only a single account to link so you know which account to use with Boom, set [Account Select](https://dashboard.plaid.com/link/account-select) to "enabled for one account" in the Plaid Dashboard. When this setting is selected, the `accounts` array will always contain exactly one account.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Boom `processor_token`. You'll send this token
to Boom and they will use it to securely retrieve transaction data from Plaid.

You can create Boom `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Boom Production credentials
...
You will need Boom Production credentials prior to initiating live traffic
in the Boom API with Plaid.
