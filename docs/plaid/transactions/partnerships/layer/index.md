---
title: "Transactions - Layer | Plaid Docs"
source_url: "https://plaid.com/docs/transactions/partnerships/layer/"
...
![Partnership Layer logo](/assets/img/docs/plaid-layer-partnership.png)

  

#### Overview

Plaid and Layer have partnered to offer small business software platforms a secure and seamless solution for integrating your SMB clients' financial activity with Layer's SMB accounting suite. Layer offers a complete accounting platform for SMBs that's directly embeddable within your platform via API and prebuilt UIs, allowing your clients to do all their bookkeeping and accounting directly within your platform. In addition to payments, bills, and embedded banking data your platform already processes on behalf of your SMB customers, Layer can use Plaid to synchronize transactions from external bank accounts to complete an SMB's financial picture.

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Layer account](https://layerfi.com/#contact-sale) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Layer `processor_token`, which allows you to quickly and securely retrieve the user's financial data via [Layer's API](https://docs.layerfi.com). Utilizing Plaid + Layer enables a seamless workflow for connecting external financial accounts to Layer.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Layer in order to use the
Plaid + Layer integration. You'll also need to enable your
Plaid account for the Layer integration.

First, you will need to work with the Layer team
to [sign up for a Layer account](https://layerfi.com/#contact-sale),
...
for Layer to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Layer to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Layer.
...
the Link `public_token` for a Plaid `access_token` and a Layer `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. The `accounts` array will contain information about bank accounts associated with the credentials entered by the user, and may contain multiple accounts if the user has more than one bank account at the institution. In most cases, you will want your users to connect all of their accounts so that they can be tracked in Layer.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Layer `processor_token`. You'll send this token
to Layer and they will use it to securely retrieve transaction data from Plaid.

You can create Layer `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Layer Production credentials
...
You will need Layer Production credentials prior to initiating live traffic
in the Layer API with Plaid.
