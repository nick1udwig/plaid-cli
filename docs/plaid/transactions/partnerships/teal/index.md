---
title: "Transactions - Teal | Plaid Docs"
source_url: "https://plaid.com/docs/transactions/partnerships/teal/"
scraped_at: "2026-03-07T22:05:22+00:00"
---

# Add Teal to your app

#### Use Teal with Plaid to connect your user's financial accounts

![Partnership Teal logo](/assets/img/docs/plaid-teal-partnership.png)

  

#### Overview

Teal seamlessly embeds accounting into your SMB platform, increasing stickiness, adding incremental revenue, and fundamentally making your SMB platform the center for their financial decisions.

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Teal account](https://docs.teal.dev/getting_started/what_is_teal) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Teal `processor_token`, which allows you to quickly and securely retrieve the user's financial data via [Teal's API](https://docs.teal.dev/getting_started/setup). Utilizing Plaid + Teal enables a seamless workflow for connecting external financial accounts to Teal.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Teal in order to use the
Plaid + Teal integration. You'll also need to enable your
Plaid account for the Teal integration.

First, you will need to work with the Teal team
to [sign up for a Teal account](https://docs.teal.dev/getting_started/what_is_teal),
...
for Teal to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Teal to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Teal.
...
the Link `public_token` for a Plaid `access_token` and a Teal `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. The `accounts` array will contain information about bank accounts associated with the credentials entered by the user, and may contain multiple accounts if the user has more than one bank account at the institution. In most cases, you will want your users to connect all of their accounts so that they can be tracked in Teal.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Teal `processor_token`. You'll send this token
to Teal and they will use it to securely retrieve transaction data from Plaid.

You can create Teal `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Teal Production credentials
...
You will need Teal Production credentials prior to initiating live traffic
in the Teal API with Plaid.
