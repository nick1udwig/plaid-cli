---
title: "Transactions - Esusu | Plaid Docs"
source_url: "https://plaid.com/docs/transactions/partnerships/esusu/"
scraped_at: "2026-03-07T22:05:21+00:00"
---

# Add Esusu to your app

#### Report positive-only rent payments to Experian, Equifax, and TransUnion with Esusu and Plaid

![Partnership Esusu logo](/assets/img/docs/plaid-esusu-partnership.png)

  

#### Overview

Plaid and Esusu have partnered to enable businesses to seamlessly offer positive-only rent reporting for their users through bank integration. Leveraging Esusu's industry-leading rent reporting services, businesses can now report current and historical rent payments to all three major credit bureaus — Equifax, TransUnion, and Experian. This partnership ensures that businesses can rely on Esusu for comprehensive services, including payment labeling, lease management, dispute management, and more, providing a robust solution for enhancing users' credit profiles.

With the Plaid + Esusu integration, businesses can use Plaid Link to instantly connect customer bank accounts and detect compliant rent payments with ease. By leveraging the Plaid processor token along with Esusu's proprietary technology, rent payments from accounts linked to your platform can be accurately identified and reported. End-users only need to link their bank account once, enabling you to offer rent reporting as a streamlined and convenient service that enhances the overall user experience.

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Esusu account](https://esusurent.com/rent-reporting-as-a-service) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Esusu `processor_token`, which allows you to quickly and securely retrieve the user's financial data via [Esusu's API](https://esusurent.com/rent-reporting-as-a-service). Utilizing Plaid + Esusu enables a seamless workflow for connecting external financial accounts to Esusu.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Esusu in order to use the
Plaid + Esusu integration. You'll also need to enable your
Plaid account for the Esusu integration.

First, you will need to work with the Esusu team
to [sign up for a Esusu account](https://esusurent.com/rent-reporting-as-a-service),
...
for Esusu to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Esusu to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Esusu.
...
the Link `public_token` for a Plaid `access_token` and a Esusu `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. The `accounts` array will contain information about bank accounts associated with the credentials entered by the user, and may contain multiple accounts if the user has more than one bank account at the institution. In most cases, you will want your users to connect all of their accounts so that they can be tracked in Esusu.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Esusu `processor_token`. You'll send this token
to Esusu and they will use it to securely retrieve transaction data from Plaid.

You can create Esusu `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Esusu Production credentials
...
You will need Esusu Production credentials prior to initiating live traffic
in the Esusu API with Plaid.
