---
title: "Transactions - Cardlytics | Plaid Docs"
source_url: "https://plaid.com/docs/transactions/partnerships/cardlytics/"
scraped_at: "2026-03-07T22:05:21+00:00"
---

# Add Cardlytics to your app

#### Turn your customers' everyday purchases into personalized cashback rewards with the Cardlytics Rewards Platform.

![Partnership Cardlytics logo](/assets/img/docs/plaid-cardlytics-partnership.png)

  

#### Overview

Plaid and Cardlytics have partnered to help publishers and retailers deliver personalized cashback rewards to their customers with ease. Cardlytics leverages purchase history to match customers with tailored cashback offers from top brands and retailers, driving loyalty, engagement, and incremental spend. With the Plaid + Cardlytics integration, partners can provide a seamless, secure rewards experience that keeps customers coming back.

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Cardlytics account](https://go.cardlytics.com/CardlyticsRewardsPlatform) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Cardlytics `processor_token`, which allows you to quickly and securely retrieve the user's financial data via [Cardlytics's API](https://platform.cardlytics.com/publisher-integrations/docs/welcome). Utilizing Plaid + Cardlytics enables a seamless workflow for connecting external financial accounts to Cardlytics.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Cardlytics in order to use the
Plaid + Cardlytics integration. You'll also need to enable your
Plaid account for the Cardlytics integration.

First, you will need to work with the Cardlytics team
to [sign up for a Cardlytics account](https://go.cardlytics.com/CardlyticsRewardsPlatform),
...
for Cardlytics to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Cardlytics to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Cardlytics.
...
the Link `public_token` for a Plaid `access_token` and a Cardlytics `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. The `accounts` array will contain information about bank accounts associated with the credentials entered by the user, and may contain multiple accounts if the user has more than one bank account at the institution. In most cases, you will want your users to connect all of their accounts so that they can be tracked in Cardlytics.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Cardlytics `processor_token`. You'll send this token
to Cardlytics and they will use it to securely retrieve transaction data from Plaid.

You can create Cardlytics `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Cardlytics Production credentials
...
You will need Cardlytics Production credentials prior to initiating live traffic
in the Cardlytics API with Plaid.
