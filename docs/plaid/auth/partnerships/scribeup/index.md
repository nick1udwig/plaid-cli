---
title: "Auth - ScribeUp | Plaid Docs"
source_url: "https://plaid.com/docs/auth/partnerships/scribeup/"
scraped_at: "2026-03-07T22:04:42+00:00"
---

# Add ScribeUp to your app

#### Embed subscription management into your digital banking experience

![Partnership ScribeUp logo](/assets/img/docs/plaid-scribeup-partnership.png)

  

#### Overview

ScribeUp and Plaid have partnered to transform how financial institutions empower their users to manage subscriptions effortlessly. By integrating Plaid’s APIs with ScribeUp’s subscription management technology, banks, credit unions, and fintechs can now offer a seamless, embedded solution within their digital banking platforms.

With the ScribeUp + Plaid integration, users gain unparalleled control over their recurring expenses. They can instantly identify subscriptions linked to their accounts or cards, uncovering payments they may not even realize they’re making. Even better, users don’t need to re-link accounts they’ve already connected in the past, creating a frictionless experience that gets them started immediately. From detecting subscriptions to canceling them with a single click, users can now manage their recurring expenses directly within your mobile or desktop banking app.

The integration also ensures users stay ahead of their recurring bills with timely notifications and calendar syncing, preventing missed payments and unnecessary fees. By embedding ScribeUp’s subscription management suite, powered by Plaid, into your banking experience, you’re not only enhancing user satisfaction but also strengthening their trust and engagement with your institution.

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [ScribeUp account](https://www.scribeup.io/partners) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a ScribeUp `processor_token`, which allows you to quickly and securely retrieve the user's financial data via [ScribeUp's API](https://docs.scribeup.io/). Utilizing Plaid + ScribeUp enables a seamless workflow for connecting external financial accounts to ScribeUp.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and ScribeUp in order to use the
Plaid + ScribeUp integration. You'll also need to enable your
Plaid account for the ScribeUp integration.

First, you will need to work with the ScribeUp team
to [sign up for a ScribeUp account](https://www.scribeup.io/partners), if you do not already have one.
...
for ScribeUp to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using ScribeUp to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact ScribeUp.
...
the Link `public_token` for a Plaid `access_token` and a ScribeUp `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. The `accounts` array will contain information about bank accounts associated with the credentials entered by the user, and may contain multiple accounts if the user has more than one bank account at the institution. In most cases, you will want your users to connect all of their accounts so that they can be tracked in ScribeUp.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a ScribeUp `processor_token`. You'll send this token
to ScribeUp and they will use it to securely retrieve account details from Plaid.

You can create ScribeUp `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid ScribeUp Production credentials
...
You will need ScribeUp Production credentials prior to initiating live traffic
in the ScribeUp API with Plaid.
