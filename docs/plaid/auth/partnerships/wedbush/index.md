---
title: "Auth - Wedbush | Plaid Docs"
source_url: "https://plaid.com/docs/auth/partnerships/wedbush/"
...
![Partnership Wedbush logo](/assets/img/docs/plaid-wedbush-partnership.png)
...
You'll need accounts at both Plaid and Wedbush in order to use the
Plaid + Wedbush integration. You'll also need to enable your
Plaid account for the Wedbush integration.

First, you will need to work with the Wedbush team
to [sign up for a Wedbush account](https://portal.wedbushfutures.com/Login), if you do not already have one.
...
for Wedbush to enable the integration.
...
the Link `public_token` for a Plaid `access_token` and a Wedbush `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. If you want the user to specify only a single account to link so you know which account to use with Wedbush, set [Account Select](https://dashboard.plaid.com/link/account-select) to "enabled for one account" in the Plaid Dashboard. When this setting is selected, the `accounts` array will always contain exactly one account.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Wedbush `processor_token`. You'll send this token
to Wedbush and they will use it to securely retrieve account details from Plaid.
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Wedbush Production credentials
...
You will need Wedbush Production credentials prior to initiating live traffic
in the Wedbush API with Plaid.
