---
title: "Auth - Riskified | Plaid Docs"
source_url: "https://plaid.com/docs/auth/partnerships/riskified/"
scraped_at: "2026-03-07T22:04:41+00:00"
---

# Add Riskified to your app

#### Provide fraud screening and ACH payment guarantee

![Partnership Riskified logo](/assets/img/docs/plaid-riskified-partnership.png)

  

#### Overview

Plaid and Riskified have partnered to offer guaranteed ACH to merchants looking to address fraud and return risk for
ACH transactions. Plaid enables consumers to instantly and securely authenticate their bank account including account
and routing number and current account balance. Merchants can leverage Plaid's Auth and Balance products in tandem with
Riskified's robust fraud and return-risk prevention technology to remain assured their ACH transactions are safe and
guaranteed.

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Riskified account](https://app.riskified.com/app/login) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Riskified `processor_token`, which allows you to quickly and securely verify a bank funding source via [Riskified's API](https://support.riskified.com/hc/en-us/articles/360019189713-Plaid-Integration-Guide) without having to store any sensitive banking information. Utilizing Plaid + Riskified enables a seamless workflow for connecting external financial accounts to Riskified.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Riskified in order to use the
Plaid + Riskified integration. You'll also need to enable your
Plaid account for the Riskified integration.

First, you will need to work with the Riskified team
to [sign up for a Riskified account](https://app.riskified.com/app/login), if you do not already have one.
...
for Riskified to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Riskified to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Riskified.
...
the Link `public_token` for a Plaid `access_token` and a Riskified `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. If you want the user to specify only a single account to link so you know which account to use with Riskified, set [Account Select](https://dashboard.plaid.com/link/account-select) to "enabled for one account" in the Plaid Dashboard. When this setting is selected, the `accounts` array will always contain exactly one account.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Riskified `processor_token`. You'll send this token
to Riskified and they will use it to securely retrieve account details from Plaid.

You can create Riskified `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Riskified Production credentials
...
You will need Riskified Production credentials prior to initiating live traffic
in the Riskified API with Plaid.
