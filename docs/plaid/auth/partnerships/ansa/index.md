---
title: "Auth - Ansa | Plaid Docs"
source_url: "https://plaid.com/docs/auth/partnerships/ansa/"
scraped_at: "2026-03-07T22:04:34+00:00"
---

# Add Ansa to your app

#### Instantly authenticate your customers' bank accounts for use with the Ansa API to enable wallet funding over ACH

![Partnership Ansa logo](/assets/img/docs/plaid-ansa-partnership.png)

  

#### Overview

Plaid and Ansa have partnered to offer businesses a frictionless, secure way for customers to add funds to their closed
loop/stored value wallets via ACH. Use Plaid Link to instantly authenticate your customers’ bank account and generate a
token that can be used to process ACH payments with Ansa. Accounts linked with Plaid can be used with
Ansa’s Payment Method, Add Balance, and Auto Reload APIs. This guide is designed for those who already have developer
accounts with both Ansa and Plaid. If that’s not you, sign up for a Plaid Dashboard account, and reach out to Ansa at
[hello@getansa.com](mailto:hello@getansa.com) to get an Ansa account.

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Ansa account](https://www.ansa.dev/) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Ansa `processor_token`, which allows you to quickly and securely verify a bank funding source via [Ansa's API](https://docs.ansa.dev) without having to store any sensitive banking information. Utilizing Plaid + Ansa enables a seamless workflow for connecting external financial accounts to Ansa.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Ansa in order to use the
Plaid + Ansa integration. You'll also need to enable your
Plaid account for the Ansa integration.

First, you will need to work with the Ansa team
to [sign up for a Ansa account](https://www.ansa.dev/), if you do not already have one.
...
for Ansa to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Ansa to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Ansa.
...
the Link `public_token` for a Plaid `access_token` and a Ansa `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. If you want the user to specify only a single account to link so you know which account to use with Ansa, set [Account Select](https://dashboard.plaid.com/link/account-select) to "enabled for one account" in the Plaid Dashboard. When this setting is selected, the `accounts` array will always contain exactly one account.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Ansa `processor_token`. You'll send this token
to Ansa and they will use it to securely retrieve account details from Plaid.

You can create Ansa `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Ansa Production credentials
...
You will need Ansa Production credentials prior to initiating live traffic
in the Ansa API with Plaid.
