---
title: "Auth - Silicon Valley Bank | Plaid Docs"
source_url: "https://plaid.com/docs/auth/partnerships/svb/"
scraped_at: "2026-03-07T22:04:43+00:00"
---

# Add Silicon Valley Bank to your app

#### Authenticate with Plaid, Pay with Silicon Valley Bank

![Partnership Silicon Valley Bank logo](/assets/img/docs/plaid-svb-partnership.png)
...
If you have an SVB ACH API-enabled account with Plaid, view the guide to begin setting up the integration. If you do not have an account with the SVB ACH API but need additional information, visit the following [SVB website](https://www.svb.com/account/fintech/plaid-integration) for more information and [SVB ACH API’s technical documentation](https://developer.svb.com/apis/instant-account-verification-v2-plaid-integration).

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Silicon Valley Bank account](https://www.svb.com/account/fintech/plaid-integration) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Silicon Valley Bank `processor_token`, which allows you to quickly and securely retrieve the user's financial data via [Silicon Valley Bank's API](https://developer.svb.com/apis/instant-account-verification-v2-plaid-integration). Utilizing Plaid + Silicon Valley Bank enables a seamless workflow for connecting external financial accounts to Silicon Valley Bank.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Silicon Valley Bank in order to use the
Plaid + Silicon Valley Bank integration. You'll also need to enable your
Plaid account for the Silicon Valley Bank integration.

First, you will need to work with the Silicon Valley Bank team
to [sign up for a Silicon Valley Bank account](https://www.svb.com/account/fintech/plaid-integration), if you do not already have one.
...
for Silicon Valley Bank to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Silicon Valley Bank to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Silicon Valley Bank.
...
the Link `public_token` for a Plaid `access_token` and a Silicon Valley Bank `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. The `accounts` array will contain information about bank accounts associated with the credentials entered by the user, and may contain multiple accounts if the user has more than one bank account at the institution. In most cases, you will want your users to connect all of their accounts so that they can be tracked in Silicon Valley Bank.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Silicon Valley Bank `processor_token`. You'll send this token
to Silicon Valley Bank and they will use it to securely retrieve account details from Plaid.

You can create Silicon Valley Bank `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Silicon Valley Bank Production credentials
...
You will need Silicon Valley Bank Production credentials prior to initiating live traffic
in the Silicon Valley Bank API with Plaid.
