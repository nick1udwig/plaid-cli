---
title: "Auth - VoPay | Plaid Docs"
source_url: "https://plaid.com/docs/auth/partnerships/vopay/"
...
![Partnership VoPay logo](/assets/img/docs/plaid-vopay-partnership.png)

  

#### Overview

[VoPay](https://vopay.com/) is a leader in payment innovation, digitizing direct bank payments with speed
and transparency. The VoPay Platform enhances how digital platforms initiate and accept financial transactions
using a single, open API. The company has one goal: to eliminate all payment inefficiencies, so businesses
can focus on what they do best in today’s fast-paced digital economy. VoPay’s EFT / ACH payment service
adds a layer of data intelligence to EFT / ACH transactions to make them instant. With the Plaid
integration, VoPay will validate and authenticate bank account information at the time of the transaction

To get started sign up for [Plaid API keys](https://dashboard.plaid.com/developers/keys).

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [VoPay account](https://vopay.com/plaid/) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a VoPay `processor_token`, which allows you to quickly and securely verify a bank funding source via [VoPay's API](https://docs.vopay.com/reference/getting-started-with-your-api) without having to store any sensitive banking information. Utilizing Plaid + VoPay enables a seamless workflow for connecting external financial accounts to VoPay.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and VoPay in order to use the
Plaid + VoPay integration. You'll also need to enable your
Plaid account for the VoPay integration.

First, you will need to work with the VoPay team
to [sign up for a VoPay account](https://vopay.com/plaid/), if you do not already have one.
...
for VoPay to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using VoPay to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact VoPay.
...
the Link `public_token` for a Plaid `access_token` and a VoPay `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. If you want the user to specify only a single account to link so you know which account to use with VoPay, set [Account Select](https://dashboard.plaid.com/link/account-select) to "enabled for one account" in the Plaid Dashboard. When this setting is selected, the `accounts` array will always contain exactly one account.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a VoPay `processor_token`. You'll send this token
to VoPay and they will use it to securely retrieve account details from Plaid.

You can create VoPay `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid VoPay Production credentials
...
You will need VoPay Production credentials prior to initiating live traffic
in the VoPay API with Plaid.
