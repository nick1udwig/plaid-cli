---
title: "Auth - ACHQ | Plaid Docs"
source_url: "https://plaid.com/docs/auth/partnerships/achq/"
...
![Partnership ACHQ logo](/assets/img/docs/plaid-achq-partnership.png)

  

#### Overview

Plaid and ACHQ have partnered to offer fast and secure processing of ACH transactions via the web and mobile.
...
You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [ACHQ account](https://achq.com/plaid) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a ACHQ `processor_token`, which allows you to quickly and securely verify a bank funding source via [ACHQ's API](http://plaid.achq.com) without having to store any sensitive banking information. Utilizing Plaid + ACHQ enables a seamless workflow for connecting external financial accounts to ACHQ.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and ACHQ in order to use the
Plaid + ACHQ integration. You'll also need to enable your
Plaid account for the ACHQ integration.

First, you will need to work with the ACHQ team
to [sign up for a ACHQ account](https://achq.com/plaid), if you do not already have one.
...
for ACHQ to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using ACHQ to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact ACHQ.
...
the Link `public_token` for a Plaid `access_token` and a ACHQ `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. If you want the user to specify only a single account to link so you know which account to use with ACHQ, set [Account Select](https://dashboard.plaid.com/link/account-select) to "enabled for one account" in the Plaid Dashboard. When this setting is selected, the `accounts` array will always contain exactly one account.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a ACHQ `processor_token`. You'll send this token
to ACHQ and they will use it to securely retrieve account details from Plaid.

You can create ACHQ `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid ACHQ Production credentials
...
You will need ACHQ Production credentials prior to initiating live traffic
in the ACHQ API with Plaid.
