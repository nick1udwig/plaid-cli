---
title: "Auth - TabaPay | Plaid Docs"
source_url: "https://plaid.com/docs/auth/partnerships/taba-pay/"
scraped_at: "2026-03-07T22:04:44+00:00"
---

# Add TabaPay to your app

#### Instantly process ACH/RTP/FedNow with TabaPay’s Unified API using your Plaid Token

![Partnership TabaPay logo](/assets/img/docs/plaid-tabapay-partnership.png)

  

#### Overview

TabaPay and Plaid have partnered together to offer streamlined pay-by-bank solutions to mutual customers. TabaPay is the leading money movement platform, transforming the payments landscape for Fintechs and financial institutions with secure, reliable, and low-cost instant payments. TabaPay provides direct access to 15 banking partners and 14 networks along with innovative products and features in one unified API. TabaPay processes over 1 million transactions daily on behalf of more than 2,500 clients in the U.S. and Canada. By integrating TabaPay's Unified API and Plaid Link, Fintechs and financial institutions can securely authenticate external bank accounts to push or pull funds via highly reliable, flexible, low-cost payment methods.

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [TabaPay account](https://developers.tabapay.com/docs/plaid) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a TabaPay `processor_token`, which allows you to quickly and securely verify a bank funding source via [TabaPay's API](https://developers.tabapay.com/docs/plaid) without having to store any sensitive banking information. Utilizing Plaid + TabaPay enables a seamless workflow for connecting external financial accounts to TabaPay.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and TabaPay in order to use the
Plaid + TabaPay integration. You'll also need to enable your
Plaid account for the TabaPay integration.

First, you will need to work with the TabaPay team
to [sign up for a TabaPay account](https://developers.tabapay.com/docs/plaid), if you do not already have one.
...
for TabaPay to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using TabaPay to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact TabaPay.
...
the Link `public_token` for a Plaid `access_token` and a TabaPay `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. If you want the user to specify only a single account to link so you know which account to use with TabaPay, set [Account Select](https://dashboard.plaid.com/link/account-select) to "enabled for one account" in the Plaid Dashboard. When this setting is selected, the `accounts` array will always contain exactly one account.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a TabaPay `processor_token`. You'll send this token
to TabaPay and they will use it to securely retrieve account details from Plaid.

You can create TabaPay `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid TabaPay Production credentials
...
You will need TabaPay Production credentials prior to initiating live traffic
in the TabaPay API with Plaid.
