---
title: "Auth - Sila Money | Plaid Docs"
source_url: "https://plaid.com/docs/auth/partnerships/sila-money/"
...
![Partnership Sila Money logo](/assets/img/docs/plaid-sila-money-partnership.png)
...
You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Sila Money account](https://console.silamoney.com) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Sila Money `processor_token`, which allows you to quickly and securely verify a bank funding source via [Sila Money's API](https://docs.silamoney.com) without having to store any sensitive banking information. Utilizing Plaid + Sila Money enables a seamless workflow for connecting external financial accounts to Sila Money.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Sila Money in order to use the
Plaid + Sila Money integration. You'll also need to enable your
Plaid account for the Sila Money integration.

First, you will need to work with the Sila Money team
to [sign up for a Sila Money account](https://console.silamoney.com), if you do not already have one.
...
for Sila Money to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Sila Money to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Sila Money.
...
the Link `public_token` for a Plaid `access_token` and a Sila Money `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. If you want the user to specify only a single account to link so you know which account to use with Sila Money, set [Account Select](https://dashboard.plaid.com/link/account-select) to "enabled for one account" in the Plaid Dashboard. When this setting is selected, the `accounts` array will always contain exactly one account.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Sila Money `processor_token`. You'll send this token
to Sila Money and they will use it to securely retrieve account details from Plaid.

You can create Sila Money `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Sila Money Production credentials
...
You can work with Sila Money in either Sandbox (sandbox.plaid.com, allows testing with simulated users) or Production (production.plaid.com, requires Sila Money Production credentials).
...
You will need Sila Money Production credentials prior to initiating live traffic
in the Sila Money API with Plaid.
