---
title: "Auth - Straddle | Plaid Docs"
source_url: "https://plaid.com/docs/auth/partnerships/straddle/"
scraped_at: "2026-03-07T22:04:43+00:00"
---

# Add Straddle to your app

#### Integrate Plaid with Straddle to deliver secure, identity-linked bank payments that settle quickly and feel as simple and reliable as using a debit card.

![Partnership Straddle logo](/assets/img/docs/plaid-straddle-partnership.png)

  

#### Overview

Plaid and Straddle have partnered to deliver a streamlined bank payment experience that prioritizes security, speed, and developer-friendly integration. When customers connect their accounts through Plaid Link, Straddle automatically confirms account ownership via Plaid’s Auth and Identity products, ensuring the account legitimately belongs to the intended user.

Once ownership is verified, Straddle’s Pay by Bank platform processes payments with built-in balance checks and continuous transaction monitoring. This means before any payment is initiated, Straddle confirms sufficient funds are available using Plaid’s Balance product, reducing the risk of failed transactions. Throughout this process, sensitive banking data never directly touches your servers.

By combining Plaid’s industry leading account authentication with Straddle’s robust payment infrastructure, businesses benefit from faster funding, fewer payment failures, and a more efficient onboarding flow. Whether you’re processing recurring subscriptions, large B2B transfers, or specialized industry payments, you can handle bank payments reliably at scale—without ever holding sensitive banking details.

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Straddle account](https://accounts.straddle.io/sign-up) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Straddle `processor_token`, which allows you to quickly and securely verify a bank funding source via [Straddle's API](https://docs.straddle.io/) without having to store any sensitive banking information. Utilizing Plaid + Straddle enables a seamless workflow for connecting external financial accounts to Straddle.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Straddle in order to use the
Plaid + Straddle integration. You'll also need to enable your
Plaid account for the Straddle integration.

First, you will need to work with the Straddle team
to [sign up for a Straddle account](https://accounts.straddle.io/sign-up), if you do not already have one.
...
for Straddle to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Straddle to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Straddle.
...
the Link `public_token` for a Plaid `access_token` and a Straddle `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. If you want the user to specify only a single account to link so you know which account to use with Straddle, set [Account Select](https://dashboard.plaid.com/link/account-select) to "enabled for one account" in the Plaid Dashboard. When this setting is selected, the `accounts` array will always contain exactly one account.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Straddle `processor_token`. You'll send this token
to Straddle and they will use it to securely retrieve account details from Plaid.

You can create Straddle `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Straddle Production credentials
...
You will need Straddle Production credentials prior to initiating live traffic
in the Straddle API with Plaid.
