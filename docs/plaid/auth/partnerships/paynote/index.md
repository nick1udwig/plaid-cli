---
title: "Auth - Paynote | Plaid Docs"
source_url: "https://plaid.com/docs/auth/partnerships/paynote/"
scraped_at: "2026-03-07T22:04:41+00:00"
---

# Add Paynote to your app
...
![Partnership Paynote logo](/assets/img/docs/plaid-paynote-partnership.png)

  

#### Overview

Paynote and Plaid have partnered to enhance eCommerce ACH payments. Utilizing Plaid's verification technology,
businesses can easily authenticate and securely store customer bank details for direct ACH debits and credits.
All transactions processed through Paynote benefit from Plaid’s Balance check and Signal features, mitigating ACH
returns. Paynote's versatile CheckoutJS and plugin options guarantee smooth integration into current checkout systems,
applications, and more. Additionally, Paynote offers subscription management, expedited funding and instant payouts
through Real-Time Payments (RTP), streamlining the financial operations of any eCommerce business.

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Paynote account](https://www.seamlesschex.com/sign-up-paynote) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Paynote `processor_token`, which allows you to quickly and securely verify a bank funding source via [Paynote's API](https://docs-paynote.seamlesschex.com/) without having to store any sensitive banking information. Utilizing Plaid + Paynote enables a seamless workflow for connecting external financial accounts to Paynote.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Paynote in order to use the
Plaid + Paynote integration. You'll also need to enable your
Plaid account for the Paynote integration.

First, you will need to work with the Paynote team
to [sign up for a Paynote account](https://www.seamlesschex.com/sign-up-paynote), if you do not already have one.
...
for Paynote to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Paynote to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Paynote.
...
the Link `public_token` for a Plaid `access_token` and a Paynote `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. If you want the user to specify only a single account to link so you know which account to use with Paynote, set [Account Select](https://dashboard.plaid.com/link/account-select) to "enabled for one account" in the Plaid Dashboard. When this setting is selected, the `accounts` array will always contain exactly one account.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Paynote `processor_token`. You'll send this token
to Paynote and they will use it to securely retrieve account details from Plaid.

You can create Paynote `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Paynote Production credentials
...
You will need Paynote Production credentials prior to initiating live traffic
in the Paynote API with Plaid.
