---
title: "Auth - Checkout.com | Plaid Docs"
source_url: "https://plaid.com/docs/auth/partnerships/checkout/"
...
![Partnership Checkout.com logo](/assets/img/docs/plaid-checkout-com-partnership.png)
...
You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Checkout.com account](mailto:partnerships@checkout.com?subject=New Plaid Checkout.com Account) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Checkout.com `processor_token`, which allows you to quickly and securely verify a bank funding source via [Checkout.com's API](https://docs.checkout.com/) without having to store any sensitive banking information. Utilizing Plaid + Checkout.com enables a seamless workflow for connecting external financial accounts to Checkout.com.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Checkout.com in order to use the
Plaid + Checkout.com integration. You'll also need to enable your
Plaid account for the Checkout.com integration.

First, you will need to work with the Checkout.com team
to [sign up for a Checkout.com account](mailto:partnerships@checkout.com?subject=New Plaid Checkout.com Account), if you do not already have one.
...
for Checkout.com to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Checkout.com to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Checkout.com.
...
the Link `public_token` for a Plaid `access_token` and a Checkout.com `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. If you want the user to specify only a single account to link so you know which account to use with Checkout.com, set [Account Select](https://dashboard.plaid.com/link/account-select) to "enabled for one account" in the Plaid Dashboard. When this setting is selected, the `accounts` array will always contain exactly one account.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Checkout.com `processor_token`. You'll send this token
to Checkout.com and they will use it to securely retrieve account details from Plaid.

You can create Checkout.com `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Checkout.com Production credentials
...
You will need Checkout.com Production credentials prior to initiating live traffic
in the Checkout.com API with Plaid.
