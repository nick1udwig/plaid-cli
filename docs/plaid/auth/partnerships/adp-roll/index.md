---
title: "Auth - Roll by ADP | Plaid Docs"
source_url: "https://plaid.com/docs/auth/partnerships/adp-roll/"
scraped_at: "2026-03-07T22:04:33+00:00"
---

# Add Roll by ADP to your app

#### Authenticate your customers’ bank accounts for secure payroll direct deposit support.

![Partnership Roll by ADP logo](/assets/img/docs/plaid-roll-by-adp-partnership.png)

  

#### Overview

Plaid and ADP now offer business partners an easier way to accelerate their payroll
money movement and reduce returns. Using Plaid Link you can instantly authenticate a
customer’s bank account, without needing to collect or provide any banking information on your own.
Once your customer's account is linked and a processor token is exchanged, ADP will be able to start money
movement to support direct deposits using ACH or RTP.

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Roll by ADP account](https://developers.adp.com/learn/learn-overview) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Roll by ADP `processor_token`, which allows you to quickly and securely verify a bank funding source via [Roll by ADP's API](https://developers.adp.com) without having to store any sensitive banking information. Utilizing Plaid + Roll by ADP enables a seamless workflow for connecting external financial accounts to Roll by ADP.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Roll by ADP in order to use the
Plaid + Roll by ADP integration. You'll also need to enable your
Plaid account for the Roll by ADP integration.

First, you will need to work with the Roll by ADP team
to [sign up for a Roll by ADP account](https://developers.adp.com/learn/learn-overview), if you do not already have one.
...
for Roll by ADP to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Roll by ADP to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Roll by ADP.
...
the Link `public_token` for a Plaid `access_token` and a Roll by ADP `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. If you want the user to specify only a single account to link so you know which account to use with Roll by ADP, set [Account Select](https://dashboard.plaid.com/link/account-select) to "enabled for one account" in the Plaid Dashboard. When this setting is selected, the `accounts` array will always contain exactly one account.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Roll by ADP `processor_token`. You'll send this token
to Roll by ADP and they will use it to securely retrieve account details from Plaid.

You can create Roll by ADP `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Roll by ADP Production credentials
...
You will need Roll by ADP Production credentials prior to initiating live traffic
in the Roll by ADP API with Plaid.
