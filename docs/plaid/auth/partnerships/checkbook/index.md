---
title: "Auth - Checkbook | Plaid Docs"
source_url: "https://plaid.com/docs/auth/partnerships/checkbook/"
...
![Partnership Checkbook logo](/assets/img/docs/plaid-checkbook-partnership.png)
...
You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Checkbook account](https://checkbook.io/register) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Checkbook `processor_token`, which allows you to quickly and securely verify a bank funding source via [Checkbook's API](https://docs.checkbook.io/reference) without having to store any sensitive banking information. Utilizing Plaid + Checkbook enables a seamless workflow for connecting external financial accounts to Checkbook.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Checkbook in order to use the
Plaid + Checkbook integration. You'll also need to enable your
Plaid account for the Checkbook integration.

First, you will need to work with the Checkbook team
to [sign up for a Checkbook account](https://checkbook.io/register), if you do not already have one.
...
for Checkbook to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Checkbook to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Checkbook.
...
the Link `public_token` for a Plaid `access_token` and a Checkbook `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. If you want the user to specify only a single account to link so you know which account to use with Checkbook, set [Account Select](https://dashboard.plaid.com/link/account-select) to "enabled for one account" in the Plaid Dashboard. When this setting is selected, the `accounts` array will always contain exactly one account.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Checkbook `processor_token`. You'll send this token
to Checkbook and they will use it to securely retrieve account details from Plaid.

You can create Checkbook `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Checkbook Production credentials
...
You will need Checkbook Production credentials prior to initiating live traffic
in the Checkbook API with Plaid.
