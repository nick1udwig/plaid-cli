---
title: "Auth - Bakkt | Plaid Docs"
source_url: "https://plaid.com/docs/auth/partnerships/bakkt/"
scraped_at: "2026-03-07T22:04:35+00:00"
---

# Add Bakkt to your app

#### Instantly authenticate your customer’s accounts to use with Bakkt Fiat Services API for ACH based money movement

![Partnership Bakkt logo](/assets/img/docs/plaid-bakkt-partnership.png)

  

#### Overview

Bakkt® offers institutional-grade custody, trading, and onramp solutions for businesses to activate crypto strategies for their users. Bakkt’s APIs power crypto trading in your business’s front-end environment, allowing your business to offer users the ability to buy, sell, and hold crypto.

  
Bakkt makes it easy, fast and secure to offer crypto trading by leveraging Plaid Link:  

1. Use the Plaid Link drop-in UI to instantly connect and authenticate user bank accounts. This generates a processor token on behalf of each user.
2. Set up Bakkt’s APIs, which use Plaid Link’s processor token to retrieve bank account details directly from Plaid. This layered integration enables a seamless workflow to connect the users’ bank accounts to Bakkt to fund crypto trading.

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Bakkt account](https://bakkt.readme.io) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Bakkt `processor_token`, which allows you to quickly and securely verify a bank funding source via [Bakkt's API](https://bakkt.readme.io/reference/generateaccesstoken ) without having to store any sensitive banking information. Utilizing Plaid + Bakkt enables a seamless workflow for connecting external financial accounts to Bakkt.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Bakkt in order to use the
Plaid + Bakkt integration. You'll also need to enable your
Plaid account for the Bakkt integration.

First, you will need to work with the Bakkt team
to [sign up for a Bakkt account](https://bakkt.readme.io), if you do not already have one.
...
for Bakkt to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Bakkt to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Bakkt.
...
the Link `public_token` for a Plaid `access_token` and a Bakkt `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. If you want the user to specify only a single account to link so you know which account to use with Bakkt, set [Account Select](https://dashboard.plaid.com/link/account-select) to "enabled for one account" in the Plaid Dashboard. When this setting is selected, the `accounts` array will always contain exactly one account.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Bakkt `processor_token`. You'll send this token
to Bakkt and they will use it to securely retrieve account details from Plaid.

You can create Bakkt `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Bakkt Production credentials
...
You will need Bakkt Production credentials prior to initiating live traffic
in the Bakkt API with Plaid.
