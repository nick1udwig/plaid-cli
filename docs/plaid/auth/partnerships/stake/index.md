---
title: "Auth - Stake | Plaid Docs"
source_url: "https://plaid.com/docs/auth/partnerships/stake/"
scraped_at: "2026-03-07T22:04:43+00:00"
---

# Add Stake to your app

#### Instantly authenticate your resident bank accounts for use with the Stake Enterprise API.

![Partnership Stake logo](/assets/img/docs/plaid-stake-partnership.png)

  

#### Overview

Plaid and Stake have partnered to create a seamless way for enterprise real estate owners and operators and Proptech companies to embed Stake’s innovative Cash Back platform and End to End Cash Back platform in their own resident-facing applications. Customers own the user experience for renters, while leveraging the efficiency and security of Plaid to connect resident bank accounts. Once a resident’s bank account is authenticated via Plaid Link, the Stake Enterprise API makes it easy to offer Cash Back to residents as a reward for completing key actions that move the needle for your business, such as paying rent on time, submitting an application, going on a tour, signing a new lease, renewing a lease, and more.

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Stake account](https://www.stake.rent/loyalty-cloud) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Stake `processor_token`, which allows you to quickly and securely verify a bank funding source via [Stake's API](https://docs.stake.rent) without having to store any sensitive banking information. Utilizing Plaid + Stake enables a seamless workflow for connecting external financial accounts to Stake.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Stake in order to use the
Plaid + Stake integration. You'll also need to enable your
Plaid account for the Stake integration.

First, you will need to work with the Stake team
to [sign up for a Stake account](https://www.stake.rent/loyalty-cloud), if you do not already have one.
...
for Stake to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Stake to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Stake.
...
the Link `public_token` for a Plaid `access_token` and a Stake `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. If you want the user to specify only a single account to link so you know which account to use with Stake, set [Account Select](https://dashboard.plaid.com/link/account-select) to "enabled for one account" in the Plaid Dashboard. When this setting is selected, the `accounts` array will always contain exactly one account.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Stake `processor_token`. You'll send this token
to Stake and they will use it to securely retrieve account details from Plaid.

You can create Stake `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Stake Production credentials
...
You will need Stake Production credentials prior to initiating live traffic
in the Stake API with Plaid.
