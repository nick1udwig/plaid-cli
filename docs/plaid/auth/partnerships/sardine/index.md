---
title: "Auth - Sardine | Plaid Docs"
source_url: "https://plaid.com/docs/auth/partnerships/sardine/"
scraped_at: "2026-03-07T22:04:41+00:00"
---

# Add Sardine to your app

#### Protect your users by using Sardine’s all-in-one fraud and compliance API

![Partnership Sardine logo](/assets/img/docs/plaid-sardine-partnership.png)

  

#### Overview

Plaid and Sardine have partnered to give businesses a more effective way to assess ACH fraud and return risk. Using Plaid's real-time balance, transaction, and identity checks, Sardine customers can better evaluate whether an ACH transaction will result in an unauthorized or NSF return. The integration between the two companies enhances Sardine's existing risk scoring capabilities, which should lead to fewer returned bank transfers and higher approval rates.

#### Getting Started

You'll first want to familiarize yourself with [Plaid Link](/plaid-link/), a drop-in client-side integration for the Plaid API that handles input validation, error handling, and multi-factor authentication. You will also need to have a verified [Sardine account](https://www.sardine.ai/contact) to add a bank funding source. Your customers will use Link to authenticate with their financial institution and select the bank account they wish to connect. From there, you'll receive a Plaid `access_token` and a Sardine `processor_token`, which allows you to quickly and securely verify a bank funding source via [Sardine's API](https://docs.sardine.ai/docs/integrate-payments/fiat-funding-api) without having to store any sensitive banking information. Utilizing Plaid + Sardine enables a seamless workflow for connecting external financial accounts to Sardine.

#### Instructions

##### Set up your accounts

You'll need accounts at both Plaid and Sardine in order to use the
Plaid + Sardine integration. You'll also need to enable your
Plaid account for the Sardine integration.

First, you will need to work with the Sardine team
to [sign up for a Sardine account](https://www.sardine.ai/contact), if you do not already have one.
...
for Sardine to enable the integration.
...
Finally, you'll need to go to the [Link customization UI](https://dashboard.plaid.com/link/data-transparency-v5) and pick the use cases that you will be using Sardine to power, so that Plaid can request the appropriate authorization and consent from your end users. If you have any questions, contact Sardine.
...
the Link `public_token` for a Plaid `access_token` and a Sardine `processor_token`.
...
user, and may contain multiple accounts if the user has more than one bank account at the institution. If you want the user to specify only a single account to link so you know which account to use with Sardine, set [Account Select](https://dashboard.plaid.com/link/account-select) to "enabled for one account" in the Plaid Dashboard. When this setting is selected, the `accounts` array will always contain exactly one account.

Once you have identified the account you will use, you will send the `access_token` and `account_id` property of the account to Plaid via the [`/processor/token/create`](/docs/api/processors/#processortokencreate) endpoint in order to create a Sardine `processor_token`. You'll send this token
to Sardine and they will use it to securely retrieve account details from Plaid.

You can create Sardine `processor_tokens` in both API environments:
...
- Production (<https://production.plaid.com>): production environment for when you're ready to go live and
  have valid Sardine Production credentials
...
You will need Sardine Production credentials prior to initiating live traffic
in the Sardine API with Plaid.
