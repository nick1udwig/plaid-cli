---
title: "Sandbox - Test credentials | Plaid Docs"
source_url: "https://plaid.com/docs/sandbox/test-credentials/"
...
These credentials have realistic transaction history and may update transactions and fire webhooks when [`/transactions/refresh`](/docs/api/products/transactions/#transactionsrefresh) is called. For more details, including information on how often new transactions are triggered or how to add your own custom transactions, see [Testing Transactions in Sandbox](/docs/transactions/#testing-transactions-in-sandbox) and [testing pending and posted transactions](/docs/transactions/transactions-data/#testing-pending-and-posted-transactions).

#### Auth micro-deposit testing credentials

Use the username `user_good` and the password `microdeposits_good`.

For more details, see [Institution details for Auth testing](/docs/sandbox/institutions/#institution-details-for-auth-testing).
...
For details on testing Bank Income with custom data, see [Testing Bank Income](/docs/income/bank-income/#testing-bank-income).

#### Monitor and Identity Verification testing users

Monitor and Identity Verification have product-specific Sandbox users that can be used for testing. For more details, see [Testing Monitor](/docs/monitor/#testing-monitor) and [Testing Identity Verification](/docs/identity-verification/testing/).
...
Note that error testing credentials are not compatible with institutions that use OAuth. If you are not sure whether an institution uses OAuth flows, see the list of [Sandbox institutions](/docs/sandbox/institutions/) to find an institution that is guaranteed not to use OAuth.
...
See [Testing Link Recovery](/docs/link/link-recovery/#testing-link-recovery).
