...
source_url: "https://plaid.com/docs/payment-initiation/variable-recurring-payments/handling-failed-payments/"
...
To retry a failed payment, call [`/payment_initiation/consent/payment/execute`](/docs/api/products/payment-initiation/#payment_initiationconsentpaymentexecute) again with the same `consent_id` and `amount`. Always ensure that retry logic doesn't create duplicate successful payments - make sure to create a new payment with a new idempotency key to ensure the retry is processed as a separate transaction, and verify the status of previous attempts before initiating new ones.
