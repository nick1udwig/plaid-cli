...
`just test-live` also includes dynamic `transactions refresh`, processor downstream reads, webhook delivery plus `webhook verification-key get`, broader Payment Initiation coverage (`recipient get/list`, `payment get/list`, `consent create/get`), and deeper Transfer coverage (`sandbox transfer fire-webhook`, `refund create/get/cancel`, `sandbox transfer refund-simulate`, recurring transfers, and sandbox test clocks). The webhook delivery test uses a disposable `webhook.site` inbox and will skip if that service is unavailable. The Transfer path self-skips when the sandbox app does not have Transfer access. `sandbox transfer sweep-simulate` is also exercised opportunistically when the sandbox state yields a sweep.
